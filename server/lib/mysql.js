...
let blogs = `create table if not exists blogs(
        ...
        blog_content VARCHAR(5000) NOT NULL COMMENT '内容',
...
let saves = `create table if not exists saves(
...
let avator = `create table if not exists avator(
        user_id VARCHAR(100) NOT NULL COMMENT '用户id',
        username VARCHAR(100) NOT NULL COMMENT '用户名',
...
let compressrecord = `create table if not exists compressrecord(
      id INT NOT NULL AUTO_INCREMENT,
      user_id VARCHAR(100) NOT NULL COMMENT '用户id',
      username VARCHAR(100) NOT NULL COMMENT '用户名',
...
let learnroad = `create table if not exists learnroad(
      id INT NOT NULL AUTO_INCREMENT,
      author VARCHAR(100) NOT NULL COMMENT '用户名',
...
let selectAllBlog = function (page) {
  let _sql = `select * from blogs order by createdate desc limit ${
    (page - 1) * 10
  },1000;`;
  return new Promise((resolve, reject) => {
    query(_sql)
        ...
        if (res.length) {
          ...
          Promise.all(getPromiseAvator).then((res2) => {
            res2.forEach((res2_ele, index) => {
              res[index].avatorurl = res2_ele.length
                ? res2_ele[0].avatorurl
                : "";
            });
            let promiseSonSql = [];
            // 遍历查询评论
...
let selectMyBlog = function (user_id, page) {
  let _sql = `select * from blogs where user_id="${user_id}" order by createdate desc limit ${
    (page - 1) * 10
  },10;`;
  return query(_sql);
};
...
// 添加学习记录
let addLearnRecord = function (value) {
  let _sql = `insert into learnroad set id=?,author=?,title=?,url=?,createdate=?;`;
  return query(_sql, value);
};
// 查询学习记录
let getLearnRecord = function () {
  let _sql = `select * from learnroad order by createdate desc;`;
  return query(_sql);
};
