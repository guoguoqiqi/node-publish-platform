...
exports.selectAllBlog = async (ctx) => {
  ...
  await userModel
    ...
    .then((res) => {
      if (res.length) {
        res.forEach((ele) => {
          ele.photos = JSON.parse(
            JSON.parse(JSON.stringify(JSON.parse(JSON.stringify(ele)).photos))
          );
        });
      }
      ctx.body = {
...
exports.uploadImage = async (ctx) => {
  ...
  upload = await new Promise((resolve, reject) => {
    fs.writeFile("./static/images/" + fileName + ".png", dataBuffer, (err) => {
      if (err) {
        reject(false);
      }
      resolve(true);
...
exports.avatorUpload = async (ctx) => {
  await check.checkToken(ctx);
  let { user_id, username, avatorFile } = ctx.request.body;
  // 将头像写入磁盘
  ...
  if (upload) {
    let avatorurl = "/images/" + getName + ".png";
...
exports.getAvator = async (ctx) => {
  ...
  if (hasAvator) {
    ...
    let readFile = await new Promise((resolve, reject) => {
      fs.readFile(
        "./static/images/" + user_id + username + ".png",
        function (err, data) {
          if (err) {
            throw err;
            reject(false);
